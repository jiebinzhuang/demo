...
public class MainUI extends JFrame {
    ...
    final private JButton btn_config = new JButton("系统初始化", image_config);
    ...
    private void init() {
        ...
        btn_config.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String[] users = {"beyonce","yao","kimkardashian","arianagrande","leomessi","jlo","kendalljenner","justinbieber",
                        "cristiano","therock","nickiminaj","neymarjr"};
                for (int i = 0; i < 12; i++) {
                    Document doc = new Document();
                    doc.put("username", users[i]);
                    doc.put(DBC.ts, DateUtils.getTS());
                    doc.put(DBC.dr, 0);
                    // 新增
                    doc.put(DBC.id, new ObjectId());
                    MongoDAO.getInstance().insert("user_popular", doc);
